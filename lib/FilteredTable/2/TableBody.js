...
var TableBody = function (_React$Component) {
    ...
    _createClass(TableBody, [{
        ...
        value: function _rowHeightGetter(idx) {
            ...
            if (table) {
                var indexOffset = noTableHeader ? 1 : 2; // aria-rowindex starts from 1 (not 0), so add 1. If there is a header, add one more since it takes aria-rowindex=1.
                var row = table.querySelector('.public_fixedDataTable_bodyRow[aria-rowindex="' + (idx + indexOffset) + '"]');

                if (row) {
                    var contents = [].concat(_toConsumableArray(row.getElementsByClassName('public_fixedDataTableCell_cellContent')));
