...
var renderCharacterLimit = function renderCharacterLimit(input, textLimit, isError) {
  return React.createElement("span", {
    className: "wfui-form-char-count ".concat(!isError ? 'text-muted' : '')
  }, "".concat(isError ? ' ' : '').concat(input && input.value ? input.value.length : 0, " / ").concat(textLimit, " characters"));
};

var renderWordLimit = function renderWordLimit(input, textLimit, isError) {
  var wordsCount = 0;

  if (input && input.value) {
    wordsCount = input.value.replace(/\n/g, ' ').split(' ').filter(function (f) {
      return f;
    }).length;
  }
  ...
  return React.createElement("span", {
    className: "wfui-form-char-count ".concat(!isError ? 'text-muted' : '')
  }, "".concat(isError ? ' ' : '').concat(wordsCount, " / ").concat(textLimit, " words"));
};
...
var renderTextArea = function renderTextArea(_ref) {
  ...
  }) : error, textLimit && !preview && renderCharacterLimit(input, textLimit, true), wordLimit && !preview && renderWordLimit(input, wordLimit, true)), (touched || showErrors) && globalError && React.createElement(Form.Control.Feedback, {
    className: "wfui-form-error",
    type: "invalid"
  ...
  }, "".concat(input && input.value ? input.value.length : 0, " / ").concat(textLimit, " characters")) : null), !((touched || showErrors) && (error || globalError)) && textLimit && !preview && renderCharacterLimit(input, textLimit), !((touched || showErrors) && (error || globalError)) && wordLimit && !preview && renderWordLimit(input, wordLimit), (touched || showErrors) && data && data.warning && React.createElement(Form.Control.Feedback, {
