...
var extLink = exports.extLink = function extLink($, _config) {
    ...
    function attach(context) {
        ...
        if (config.extExclude) {
            if (typeof config.extExclude === 'string') {
                ...
                var extExcConcat = '(' + config.extExclude.join('|') + ')';
            ...
            if ($link.css('display') === 'inline' || $link.css('display') === 'inline-block' || $link.css('display') === 'block') {
                if (config.extSpanClass && config.extSpanClass.length > 0) {
                    if (config.extSpanClass === config.mailtoClass) {
                        $link.append('<span class="' + config.extSpanClass + '"><span class="element-invisible sr-only"> ' + config.mailtoLabel + '</span></span>');
                    } else {
                        $link.append('<span class="' + config.extSpanClass + '"><span class="element-invisible sr-only"> ' + config.extLabel + '</span></span>');
                    }
                }
            }
        }
    ...
    return { attach: attach, manualConfirm: manualConfirm };
};