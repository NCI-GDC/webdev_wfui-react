    ...
    this.state = {
      ...
      dataWithClass: props.data
        ? props.data.map((item) => {
            ...
            newItem.className = `nav-item-${item[props.itemIdField]} ${item[props.itemIdField] ===
            props.cascNav
              ? 'active'
              : ''}`;
            return newItem;
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (cascNav) {
      if (!nextProps.cascNav) {
        window.location.href = `${window.location.href.split('?')[0]}?cascNav=${nextProps.data[0][
  ...
  render() {
    ...
    if (visibilityField) {
      navFormat.push({
        ...
        display: item => (item[visibilityField] ? '' : <Glyphicon className="eye-close" />),
      });
    }
