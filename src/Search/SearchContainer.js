...
class SearchContainer extends React.Component {
    ...
    componentWillMount() {
        ...
        if (location.query && location.query.q) {
            this.props.getSearchResult(config, { keyword: location.query.q, typeInclude, keywordQuotes });
        } else {
            this.props.getSearchResult(config, { keyword: '', typeInclude, keywordQuotes });
        }
    }
    ...
    searchSubmit(term) {
        const { onSearchSubmit, config, typeInclude, keywordQuotes } = this.props;
        this.state = {
            fetching: false,
            autoFetched: false,
            secondsElapsed: 0,
        ...
        this.props.getSearchResult(config, { keyword: term.trim(), typeInclude, keywordQuotes });
        ...
        if (!fetching) {
            if (secondsElapsed > 3) {
                if (term.length > 0 && autoTerm !== term) {
                    ...
                    this.props.getAutocomplete(config, { keyword: term, typeInclude }).then(() => {
                        this.setState({
                            fetching: false,
                            autoFetched: true,
                        });
    ...
    render() {
        const { autocomplete, placeholder, textSubmit, textNoResult } = this.props;
        ...
        return (
            <div className="wfui-search-container">
                ...
                />
                { fetching && (
                    <ul id="ui-id-1" className="ui-menu ui-widget ui-widget-content ui-autocomplete ui-front"><li className="mp ps"><Spinner type={1} color="#0072c6" fontSize={`5px`} margin={`10px auto`} /></li></ul>
                )}
                { autoFetched && startAutosearch && <SearchAutocomplete textNoResult={textNoResult} autocomplete={autocomplete} onClickTerm={(e) => { this.searchSubmit(e.target.getAttribute('data-keyword')); }} />}
            </div>
        );
    }
}
