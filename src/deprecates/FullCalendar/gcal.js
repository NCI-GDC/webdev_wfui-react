...
(function(factory) {
    ...
    FC.sourceNormalizers.push(function(sourceOptions) {
        let { googleCalendarId } = sourceOptions;
    ...
    function transformOptions(sourceOptions, start, end, timezone, calendar) {
        const url = `${API_BASE}/${encodeURIComponent(
            sourceOptions.googleCalendarId
        )}/events?callback=?`; // jsonp
        const apiKey =
            sourceOptions.googleCalendarApiKey ||
            calendar.options.googleCalendarApiKey;
        const { success } = sourceOptions;
        let data;
        let timezoneArg; // populated when a specific timezone. escaped to Google's liking

        function reportError(message, apiErrorObjs) {
            const errorObjs = apiErrorObjs || [{ message }]; // to be passed into error handlers

            // call error handlers
            (sourceOptions.googleCalendarError || $.noop).apply(
                calendar,
                errorObjs
            );
