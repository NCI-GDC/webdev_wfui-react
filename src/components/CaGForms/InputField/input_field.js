...
class InputField extends Component {
    ...
    render() {
        ...
        if (prefix) {
            var prefixField = (
                <span className="wfui-input-field__prefix">{prefix}</span>
            );
        }
        if (postfix) {
            var postfixField = (
                <span className="wfui-input-field__postfix">{postfix}</span>
            );
        }
        // check error flag
        let errorClassName = '';
        ...
        const inputFieldElement = hideField ? (
            ''
        ) : (
            <span>
                {prefixField}
                <input
                    className={`wfui-input-field__input${errorClassName}`}
                    type={type}
